...
namespace CompiPascalC3D.Analizer.Languaje.Expressions
{
    class Literal : Expresion
    {
        ...
        public override Returned Execute(Ambit ambit)
        {
            ...
            if (this.type == 1)
            {
                returned = new Returned(this.value.ToString(), DataType.INTEGER, false, literal_string,  this.value.ToString());
            }
            else if (this.type == 2)
            {
                ...
                returned = new Returned(temp, DataType.STRING, true, literal_string, this.value.ToString());
            }
            ...
            else if (this.type == 3)
            {
                ...
                if (this.value.ToString() == "false")
                {
                    ...
                    returned = new Returned("false", DataType.BOOLEAN, false, this.TrueLabel, this.FalseLabel, literal_string, "false");
                }
                else
                {
                    literal_string += generator.add_Goto(this.TrueLabel, cant_tabs);
                    returned = new Returned("true", DataType.BOOLEAN, false, this.TrueLabel, this.FalseLabel, literal_string, "true");
                }
            }
            else if (this.type == 4)
            {
                returned = new Returned(this.value.ToString(), DataType.REAL, false, literal_string, this.value.ToString());
            }

            else if (this.type == 7)
            {
                returned = new Returned(this.value.ToString(), DataType.IDENTIFIER, false, literal_string, this.value.ToString());
            }
            return returned;
